...
import (
	...
	"github.com/Medveddo/rocket-science/platform/pkg/testcontainers/app"
	"github.com/Medveddo/rocket-science/platform/pkg/testcontainers/mongo"
...
func setupTestEnvironment(ctx context.Context) *TestEnvironment {
	logger.Info(ctx, "🚀 Подготовка тестового окружения для inventory service...")
	...
	appEnv := map[string]string{
		// Добавляем все необходимые переменные окружения для inventory service
...
// getEnvWithLogging возвращает значение переменной окружения с логированием и дефолтными значениями
func getEnvWithLogging(ctx context.Context, key string) string {
	value := os.Getenv(key)
	if value == "" {
		// Устанавливаем дефолтные значения для inventory service
		switch key {
		case testcontainers.MongoUsernameKey:
			value = "admin"
		case testcontainers.MongoPasswordKey:
			value = "password"
		case testcontainers.MongoImageNameKey:
			value = "mongo:7.0"
		case testcontainers.MongoDatabaseKey:
